...
//
// ============================================================================
package org.talend.components.filesystem.runtime.test;
...
/**
 * Reusable for creating a {@link JavaSparkContext} pointing to a specific cluster for Spark integration tests.
 */
public class SparkIntegrationTestResource extends TemporaryFolder {
    ...
    public final static Map<String, String> HADOOP_CONF_DEFAULT = ImmutableMap.<String, String> builder()
            .put(CommonConfigurationKeys.FS_DEFAULT_NAME_KEY, NN_DEFAULT).put(YarnConfiguration.RM_ADDRESS, RM_DEFAULT)
     ...
     */
    private SparkIntegrationTestResource(String nn, String rm, String rms, String sm, boolean isKerberos,
        ...
        this.sm = sm;
        if (isKerberos) {
            this.hadoopConf = ImmutableMap.<String, String> builder().putAll(hadoopConf)
                    .put("hadoop.security.authentication", "kerberos").put("hadoop.security.authorization", "true").build();
        } else {
            this.hadoopConf = hadoopConf;
        }
    }

    public static SparkIntegrationTestResource ofYarnClient(String nn, String rm, String rms, boolean isKerberos,
            Map<String, String> hadoopConf) {
        return new SparkIntegrationTestResource(nn, rm, rms, "yarn-client", isKerberos, hadoopConf);
    }

    public static SparkIntegrationTestResource ofDefault() {
        return new SparkIntegrationTestResource(NN_DEFAULT, RM_DEFAULT, RMS_DEFAULT, SM_DEFAULT, false, HADOOP_CONF_DEFAULT);
    }

    public static SparkIntegrationTestResource ofKerberosDefault() {
        return new SparkIntegrationTestResource(NN_DEFAULT, RM_DEFAULT, RMS_DEFAULT, SM_DEFAULT, true, HADOOP_CONF_DEFAULT);
    }

    public static SparkIntegrationTestResource ofLocal() throws IOException, URISyntaxException {
        Configuration configuration = new Configuration();
        configuration.set(MiniDFSCluster.HDFS_MINIDFS_BASEDIR, "/tmp/localfs");
        MiniDFSCluster.Builder builder = new MiniDFSCluster.Builder(configuration);
        MiniDFSCluster hdfsCluster = builder.build();
        return new SparkIntegrationTestResource(configuration.get("fs.defaultFS"), "local", "local", "local[2]", false,
    ...
    public SparkContextOptions getOptions() {
        if (options == null) {
            ...
            options.as(HadoopFileSystemOptions.class).setHdfsConfiguration(Arrays.asList(createHadoopConfiguration()));
        }
        return options;
    }
