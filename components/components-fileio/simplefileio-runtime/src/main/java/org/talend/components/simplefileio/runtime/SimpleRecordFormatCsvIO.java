...
public class SimpleRecordFormatCsvIO extends SimpleRecordFormatBase {
    ...
    public PDone write(PCollection<IndexedRecord> in) {
        ...
        if (path.startsWith("gs://")) {
            ...
            PCollection<String> pc1 = in.apply("FormatCSVRecord", ParDo.of(new FormatCsvRecord2(fieldDelimiter.charAt(0))));

            if (in.isBounded() == PCollection.IsBounded.BOUNDED) {
                return pc1.apply(TextIO.write().to(path));
            } else {
                pc1 = UnboundedWrite.ofDefaultWindow(pc1);
                WriteFilesResult results = pc1.apply(FileIO.<String> write().withNumShards(1).via(TextIO.sink()).to(path));
            ...
            UgiFileSinkBase<NullWritable, Text> sink = new UgiFileSinkBase<>(doAs, path, overwrite, mergeOutput,
