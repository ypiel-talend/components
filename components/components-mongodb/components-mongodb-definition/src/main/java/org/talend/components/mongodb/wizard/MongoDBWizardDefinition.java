...
public class MongoDBWizardDefinition extends AbstractComponentWizardDefintion {

    public static final String COMPONENT_WIZARD_NAME = "mongodb"; //$NON-NLS-1$
    ...
    public ComponentWizard createWizard(String location) {
        return new MongoDBWizard(this, location);
    }

    @Override
    public boolean supportsProperties(Class<? extends ComponentProperties> propertiesClass) {
        return propertiesClass.isAssignableFrom(MongoDBConnectionProperties.class);
    }

    @Override
    public ComponentWizard createWizard(ComponentProperties properties, String location) {
        MongoDBWizard wizard = (MongoDBWizard) createWizard(location);
        wizard.setupProperties((MongoDBConnectionProperties) properties);
        return wizard;
    }
    ...
    public String getImagePath(DefinitionImageType type) {
        switch (type) {
        case TREE_ICON_16X16:
            return "nosqlConnectionsNode.gif";
        case WIZARD_BANNER_75X66:
            return "nosqlConnectionsWizard.png";
        default:
            // will return null
        }
        return null;
